...
The agent also tracks per-user preferences (`UserProfileConfig`), remembers user requests (`UserMemoryConfig`), and maintains session state (`SessionContextConfig`).
...
```sh
mise run agent:cli -- data_agent -q "How many races has Lewis Hamilton won?"
mise run agent:cli -- data_agent  # interactive mode
```
