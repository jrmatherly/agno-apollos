...
| DevOps | No MCP scopes. MCP endpoints return 403. |
...
The Settings > MCP Integrations page (`/settings/integrations`) displays registered MCP servers with their status, URLs, and management actions. Admins can register new servers and remove existing ones through the UI.
