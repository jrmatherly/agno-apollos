...
| Variable | Default | Description |
|----------|---------|-------------|
| `JWT_SECRET_KEY` | (empty) | HMAC secret for JWT auth. Empty = auth disabled |

When `JWT_SECRET_KEY` is set, the backend requires a valid JWT token on all API requests. See [security configuration](/configuration/security) for details.

## Observability

| Variable | Default | Description |
|----------|---------|-------------|
| `OTEL_EXPORTER_OTLP_ENDPOINT` | (empty) | OTLP HTTP endpoint for trace export. Empty = disabled |

When set, agent traces are exported to the specified OpenTelemetry collector. See [telemetry configuration](/configuration/telemetry) for details.
