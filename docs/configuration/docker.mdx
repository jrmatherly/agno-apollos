...
Apollos AI runs as three core Docker services managed by Docker Compose, plus optional services behind profiles.
...
| `apollos-m365-mcp` | 9000 | `node:24-alpine` (npx) | `m365` (opt-in) |
| `apollos-mcp-gateway` | 4444 | `ghcr.io/ibm/mcp-context-forge:1.0.0-RC1` | `gateway` (opt-in) |
| `pgadmin` | 5050 | `dpage/pgadmin4` | `tools` (opt-in) |
...
```bash
mise run docker:up --docs                    # docs service
mise run docker:up --m365                    # M365 MCP server
mise run docker:up --gateway                 # MCP Gateway (ContextForge)
mise run docker:up --m365 --gateway --docs   # all optional services
mise run docker:up --tools                   # pgAdmin database UI
```
...
```bash
mise run dev
mise run dev --m365 --gateway   # watch mode with optional services
```
...
| Include M365 MCP server | `mise run docker:up --m365` |
| Include MCP Gateway | `mise run docker:up --gateway` |
| Stop all services | `mise run docker:down` (`--prod`, `--m365`, `--gateway`, `--docs`) |
| Tail logs | `mise run docker:logs` (`--prod`, `--m365`, `--gateway`, `--docs`) |
| Start database only | `mise run db` |
| Build images | `mise run docker:build` |
| Start docs standalone | `mise run docs:docker` |
| Start gateway standalone | `mise run gateway:up` |
| Tail gateway logs | `mise run gateway:logs` |
