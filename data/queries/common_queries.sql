...
FROM
    ...
    AND dc.year = EXTRACT(
        YEAR
        FROM TO_DATE (rw.date, 'DD Mon YYYY')
    )
WHERE
    dc.position = '1' -- TEXT comparison for drivers_championship
GROUP BY
    dc.year,
    dc.name
...
-- <query>
WITH
    race_wins_by_year AS (
        SELECT team, COUNT(*) AS wins
        FROM race_wins
        WHERE
            EXTRACT(
                YEAR
                FROM TO_DATE (date, 'DD Mon YYYY')
            ) = 2019
        GROUP BY
            team
    )
SELECT cc.position, cc.team, cc.points, COALESCE(rw.wins, 0) AS race_wins
FROM
    constructors_championship cc
...
-- </query description>
-- <query>
SELECT name AS driver, COUNT(*) AS total_wins
FROM race_wins
GROUP BY
    name
...
WHERE
    position = '1' -- TEXT comparison (note the quotes!)
GROUP BY
    name
ORDER BY championship_wins DESC
LIMIT 10
...
FROM race_results
WHERE
    position IN ('1', '2', '3') -- TEXT comparison
GROUP BY
    name
...
-- </query description>
-- <query>
SELECT year, COUNT(*) AS retirements
FROM race_results
WHERE
    position = 'Ret'
GROUP BY
...
-- </query description>
-- <query>
SELECT name AS driver, name_tag, COUNT(*) AS wins
FROM race_wins
WHERE
    venue ILIKE '%Monaco%'
GROUP BY
...
SELECT
    ...
    MAX(
        CASE
            WHEN team = 'Ferrari' THEN points
        END
    ) AS ferrari_points,
    MAX(
        CASE
            WHEN team = 'Mercedes' THEN points
        END
    ) AS mercedes_points
FROM constructors_championship
WHERE
    team IN ('Ferrari', 'Mercedes')
...
FROM race_results
WHERE
    position ~ '^[0-9]+$' -- Only numeric positions
